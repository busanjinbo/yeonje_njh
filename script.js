document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleHashChange() {
        ...
        if (targetSect) {
            ...
            const ytPledge4Container = document.getElementById('youtube-pledge4-box');
            if (ytPledge4Container) {
                if (targetId === 'view-pledge4') {
                    if (ytPledge4Container.innerHTML.trim() === '' || ytPledge4Container.innerHTML.indexOf('iframe') === -1) {
                        ytPledge4Container.innerHTML = `<iframe width="100%" height="100%" src="${ytPledge4Container.getAttribute('data-src')}" title="진보구청장 노정현 공약 영상" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen style="border-radius:20px; box-shadow:0 10px 30px rgba(0,0,0,0.1); aspect-ratio:16/9;"></iframe>`;
                    }
                } else {
                    ytPledge4Container.innerHTML = '';
                }
            }
        }
